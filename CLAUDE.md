...
## Google Analytics
- **測定ID**: `G-3TCK4GZRR2`
- mbtidemoと**同じGAプロパティ**で、**データストリームを分けて**運用（サイト横断のユーザー導線を追跡可能）
- `index.html` と `manga/index.html` の `<head>` 内に gtag スニペットを設置済み

## トップページ ヒーローセクション仕様
- `yumeiro_jissya/` フォルダ内の16体の実写べあーず画像を使用
- **配置**: `top-image/yonde_market_logo.png` のように、ページ中心を囲むように円形に配置（頭が中心を向く）
- **アニメーション**: ページ読み込み時に16体が四方からわっと集まってくる演出
- **スクロール連動**: スクロールするとべあーずがフェードアウトして消える
- **スクロールインジケーター**: 右側に「スクロールしてね」を縦書き表示
- ※現在の実装は4x4グリッド配置のため、円形配置に修正が必要

## 表記ルール
- サイト内テキストでは「くま」ではなく「べあーず」を使用する

## ファイル構成（追加分）
```
yumeiro_jissya/         - 16体の実写べあーず画像（MBTI型名.png）
top-image/              - トップページ参考画像
  yonde_market_logo.png - べあーず配置の参考画像
```
